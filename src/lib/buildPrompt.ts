...
export function buildSystemPrompt(
  ...
  // ── Existing characters ──────────────────────────────────────────────────
  const existingCharactersStr =
    gameState.characters.length > 0
      ? gameState.characters
          .map(c => `  • id="${c.id}" name="${c.name}" — ${c.traits[0] ?? 'роль неизвестна'}`)
          .join('\n')
      : '  • Персонажей ещё нет.';
...
${existingCharactersStr}
Если персонаж из этого списка снова появляется в сцене — используй его существующий id и выставь "isUpdate": true. Не создавай дубликаты.
...
${isFinalPhase
  ...
  : `[Нарративный текст: МИНИМУМ 4 абзаца, каждый абзац — минимум 3 предложения. Описывай сцену богато: окружение, детали, эмоции персонажей, атмосферу. Сначала реагируй на действие игрока, затем развивай сцену.]`}
</NARRATIVE>
...
[
  {
    ...
    "name": "Имя персонажа",
    "role": "Роль (например: торговец, стражник, маг)",
    "description": "1–2 предложения биографии или предыстории персонажа.",
    "lastInteraction": "Краткое описание последнего взаимодействия с героем.",
    "isUpdate": false
  }
]
</CHARACTERS>

Правила для <CHARACTERS>:
  • Добавляй персонажа ("isUpdate": false) только при ПЕРВОМ значимом взаимодействии — диалог, конфликт, совместное действие. Случайных статистов не добавляй.
  • Если персонаж уже есть в разделе ЗАРЕГИСТРИРОВАННЫЕ ПЕРСОНАЖИ — ОБЯЗАТЕЛЬНО используй его точный id и выставь "isUpdate": true. Никогда не создавай нового персонажа с другим id если он уже зарегистрирован.
  • Обновляй ("isUpdate": true) если изменилось отношение, роль или произошло важное событие.
  • Если новых/обновлённых персонажей нет — верни пустой массив: [].
  • id должен быть латинскими буквами без пробелов, уникальным для каждого персонажа.
